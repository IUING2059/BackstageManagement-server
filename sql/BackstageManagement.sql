-- 创建数据库，不存在则创建
CREATE DATABASE IF NOT EXISTS backstage_management DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- 切换到该数据库
USE backstage_management;
...
create table clazz (
    ...
    constraint uk_clazz_name unique (name)
) comment '班级表' engine=InnoDB default charset=utf8mb4;
...
create table emp (
    ...
    constraint uk_emp_username unique (username)
) comment '员工表' engine=InnoDB default charset=utf8mb4;
...
) comment '工作经历' engine=InnoDB default charset=utf8mb4;
...
) comment '员工日志表' engine=InnoDB default charset=utf8mb4;
...
create table student (
    ...
    create_time     datetime                     null comment '创建时间',
    update_time     datetime                     null comment '修改时间',
    constraint uk_student_id_card unique (id_card),
    constraint uk_student_no unique (no),
    constraint uk_student_phone unique (phone)
) comment '学员表' engine=InnoDB default charset=utf8mb4;
